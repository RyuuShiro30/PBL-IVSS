...
try {
    ...
    $check_stmt = $pdo->prepare("SELECT COUNT(*) as total FROM admin_berita WHERE username = ?");
    ...
    $check_stmt = $pdo->prepare("SELECT COUNT(*) as total FROM admin_berita WHERE email = ?");
    ...
    $stmt = $pdo->prepare("
        INSERT INTO admin_berita (username, password, nama_lengkap, email, role, foto, status, created_at, updated_at) 
        VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
    ");
